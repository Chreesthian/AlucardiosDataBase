...
def refresh_file(engine, path: Path, *, kind: str = "dump", allow_empty: bool = False) -> dict:
    ...
    return refresh(engine, parsed, kind=kind, source_path=str(path), allow_empty=allow_empty)


def refresh_text(engine, texto: str, *, kind: str = "dump", allow_empty: bool = False) -> dict:
    return refresh(engine, parse_text(texto), kind=kind, source_path=None, allow_empty=allow_empty)
...
def _una_pasada(engine, path: Path, exportar: bool, allow_vacio: bool = False) -> dict:
...
def main() -> None:
    ...
    while True:
        try:
            if not path.exists():
                log.warning("volcado ausente: %s; reintento en %ss", path, args.interval)
            else:
                huella = _huella(path)
                if huella != ultima_huella:
                    ...
                    log.info("volcado sin cambios; próxima comprobación en %ss", args.interval)
